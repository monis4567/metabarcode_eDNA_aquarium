...
inp_fn1 <- "part04_blast_blibnumber_Lnumber.results.01.txt"
...
outputfilenm1="part05_blast_blibnumber_Lnumber.results.02.txt"
...
#lst_posibl_spc6 <- read.delim(file=inp_fn6,sep=' ',header=F, fill=T)
#only retain 1st and second column - this will help you work around any eventual subspecies names
...
#lst_posibl_spc6 <- lst_posibl_spc6[,1:2]
#bind rows together
lst_posibl_spc <- rbind(lst_posibl_spc2,
                        lst_posibl_spc3,
                        lst_posibl_spc4,
                        lst_posibl_spc5)
#instead of using several different lists with species, the alternative is to
# perhpas just use one single list
#lst_posibl_spc <- read.delim(file=inp_fn6,sep=' ',header=F, fill=T)
