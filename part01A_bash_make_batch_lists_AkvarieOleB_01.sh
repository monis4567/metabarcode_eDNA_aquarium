...
do
	#iterate over L array
 	...
 	do	
		...
		FASTQF01=$(ls "${INDIR02}" | grep "$fn" | grep "$fl" | grep '_1')
		#echo "$FASTQF01"
		#FASTQF02=$(ls "${INDIR02}" | grep "$fn" | grep "$fl" | grep '_2' | sed -E 's/\.fq//g')
		FASTQF02=$(ls "${INDIR02}" | grep "$fn" | grep "$fl" | grep '_2')
		#echo "$FASTQF02"
		# make an output file name with the number iterated over
		OUTLSTF01=$(echo "part01A_batch_file_AkvarieOleB_b"${fn}"_"${fl}".list")
...
" > "${PATH01}"/"${BDR}"/"${OUTLSTF01}"
	...
	#end iteration over fl
	done
	
#end iteration over fn
done
...
#SBATCH -c 1
#SBATCH -t 96:00:00
#SBATCH -J pa01B_demulti_b"${fn}_${fl}"
#SBATCH -o stdout_pa01B_demulplex_b"${fn}_${fl}".txt
#SBATCH -e stderr_pa01B_demulplex_b"${fn}_${fl}".txt


## Activating the shell script: 'part01B_dada2_demultiplexing_v3_b"${fn}_${fl}".sh  
## will call upon two additional files as input files for the demultiplex process.
## In this case these files are :
## "part01A_batch_file_AkvarieOleB_b"${fn}_${fl}".list"
## and
## part01B_tags.txt
## Make sure these files also are available in the same directory as where you place
## the file 'part01B_dada2_demultiplexing_v3_b"${fn}_${fl}".sh'
...
srun part01B_dada2_demultiplexing_v3_b"${fn}_${fl}".sh" > "${PATH01}"/"${BDR}"/part01B_sbatch_run_dada2_demultiplexing_v3_b"${fn}_${fl}".sh
...
do
	#iterate over L array
 	...
 	do	
		...
		sed "s/Lnumber/"${fl}"/g" > "${PATH01}"/"${BDR}"/part01B_dada2_demultiplexing_v3_b"${fn}_${fl}".sh
		#make the DADA2 sh script executable
		chmod 755 "${PATH01}"/"${BDR}"/part01B_dada2_demultiplexing_v3_b"${fn}_${fl}".sh
	...
	#iterate over L array
 	for fl in \${ARRL[@]}
 		do	
			...
			cd "${PATH01}"/\"\${BDR}\"
			sbatch part01B_sbatch_run_dada2_demultiplexing_v3_b\"\${fn}\"_\"\${fl}\".sh
			cd "${WD}"
		done
	done
...
After you have run ./part01A_bash_make_batch_lists_AkvarieOleB_01.sh 
Then execute:
